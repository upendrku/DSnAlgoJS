...
function minSubArrayLen(arr, sum) {
    let requiredNoOfEle = Infinity
    let tempSum = 0
    let i=0, j=0
    while(i < arr.length) {
        if(tempSum < sum && j < arr.length) {
            tempSum += arr[j]
            j++
        } else if(tempSum >= sum) {
            tempSum -= arr[i]
            requiredNoOfEle = Math.min(requiredNoOfEle, j-i)
            i++
        } else {
            break;
        }
    }
    console.log(requiredNoOfEle === Infinity ? 0 : requiredNoOfEle)
}
